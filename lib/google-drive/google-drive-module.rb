module GoogleDriveModule
  ...
  def self.update_competitor_prices
    ...
    worksheet_today   = spreadsheet.worksheets.first
    ...
    row = 2
    while worksheet_today[row, 1] != ''
      ean     = worksheet_today[row, 1]
      puts Time.now.to_s + ' Updating EAN: ' + ean
      
      prices  = PriceScraperModule.scrape_ean(ean)

      GoogleDriveModule.update_today(row, prices, worksheet_today)
